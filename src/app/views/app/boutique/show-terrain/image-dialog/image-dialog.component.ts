...
export class ImageDialogComponent implements OnInit, AfterViewInit {
  ...
  saveImageSelect() {
    let path: any = {image: this.imageSelected};
  ...
  async uploadFile(f?) {
    ...
    if (this.file) {
      const filePath = `${this.basePath}/${Math.random()}${this.file.name}`;    //path at which image will be stored in the firebase storage
      const snap = await this.af.upload(filePath, this.file);    //upload task
      this.getUrl(snap);
