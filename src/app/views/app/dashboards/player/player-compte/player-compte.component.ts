...
@Component({
  selector: 'app-player-compte',
  templateUrl: './player-compte.component.html',
  styleUrls: ['./player-compte.component.scss']
})
export class PlayerCompteComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.authenticationService.getConnectedPlayer().subscribe((res: any) => {
      console.log(res);
      ...
      this.profile.profile.rightFooted = res.user?.rightFooted;
      this.profile.profile.position = res.user?.position;
      this.profile.profile.shirtNumber = res.user?.shirtNumber;
      this.profile.profile.Team = res.user?.Team;
      this.selectedItem = this.positionList.find(p => p.abbreviation == this.profile.profile.position)?.value ?? this.positionList[0]?.value;
  ...
  changedPosition(e){
    this.profile.profile.position = this.positionList.find(p => p.value == this.selectedItem)?.abbreviation ?? this.positionList[0]?.abbreviation;
    // console.log(e.target.value);
    console.log(this.profile.profile.position);
